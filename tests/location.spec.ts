...
test("TC-DA-LOC-002 : ตรวจสอบแสดงข้อมูลเมื่อกดค้นหา", async ({ page }) => {
  ...
  const geo = GEO_CHIANG_MAI;

  console.log(
    `📍 Mock ตำแหน่ง: ${geo.latitude}, ${geo.longitude} | จังหวัด: "${geo.province}"`,
  );
  ...
  console.log(
    `📦 API totalCount: ${searchData.totalCount}, items: ${searchData.items?.length ?? 0}`,
  );

  expect(searchData.items?.length ?? 0).toBeGreaterThan(0);

  const searchProvince = geo.province;
  ...
  console.log(`✅ UI แสดง ${cardCount} cards`);
});
...
test("TC-DA-LOC-006 : ตรวจสอบรองรับ decimal", async ({ page }) => {
  ...
  const { latitude: testLat, longitude: testLng, province } = GEO_CHIANG_MAI;
  const searchProvince = province;
...
test("TC-DA-LOC-017 : ตรวจสอบแสดงเวลาค้นหา", async ({ page }) => {
  ...
  const windowStartSeconds =
    locationPage.toUtcPlus7Seconds(searchStart) - bufferSeconds;
  const windowEndSeconds =
    locationPage.toUtcPlus7Seconds(searchLoaded) + bufferSeconds;
...
test("TC-DA-LOC-025 : ตรวจสอบการค้นหาเมื่อเลือกตัวกรองขั้นสูงหลายตัวพร้อมกัน", async () => {
  ...
  console.log(
    `✅ Validated ${allItems.length} API items | All filters matched — ผ่าน`,
  );
});
...
test("TC-DA-LOC-028 : Filter แล้วไม่พบข้อมูล", async () => {
  ...
  const eventOptionValue = await locationPage
    .eventDropdown()
    .evaluate((el, keyword) => {
