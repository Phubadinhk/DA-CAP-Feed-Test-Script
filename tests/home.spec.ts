...
import { HOME_DATA, TEST_CAP_ITEM } from "../test-data/home.data";
...
test.describe("Home Page - การทดสอบฟังก์ชันการค้นหาและแสดงผลข้อมูลทั่วประเทศ", () => {
  ...
  test("TC-DA-HP-013 - ตรวจสอบ reset filter", async ({ page }) => {
    ...
    const filteredResponsePromise = home.waitForCountryListResponse(
      (response) => response.url().includes(`eventId=${selectedEventId}`),
    );
  ...
  test("TC-DA-HP-015 - Filter แล้วไม่พบข้อมูล", async () => {
    ...
    const noResultEventId = HOME_DATA.EVENT_IDS.NO_RESULT; // 47

    await home.openAdvancedFilter();
    await home.selectEventByNativeSelect(selectedEvent);

    const selectedEventId = await home.getSelectedEventId();
    expect(selectedEventId).toBe(String(noResultEventId));
    ...
    const searchResponsePromise = home.waitForCountryListResponse(
      (response) => {
        ...
        return (
          url.pathname.includes(HOME_DATA.API.COUNTRY_LIST) &&
          url.searchParams.get("eventId") === String(noResultEventId) &&
          response.status() === 200
        );
      },
    );
  ...
  test("TC-DA-HP-019 - ตรวจสอบสี Color Dot ความรุนแรงร้ายแรงมาก", async () => {
    ...
    for (let index = 0; index < cardCount; index += 1) {
      const cardBody = home.cards
        .nth(index)
        .locator(HOME_DATA.SELECTORS.cardBody);
      const hasSeverity =
        (await cardBody.getByText(HOME_DATA.FILTERS.severity[0]).count()) > 0;

      if (!hasSeverity) continue;

      const severityText = cardBody
        .getByText(HOME_DATA.FILTERS.severity[0])
        .first();
  ...
  test("TC-DA-HP-025 - ตรวจสอบลิงก์ .XML เปิดได้", async ({ page }) => {
    // ใช้ test data ที่กำหนดไว้
    const xmlUrl = TEST_CAP_ITEM.xmlLink;
    console.log(`📌 xmlUrl: ${xmlUrl}`);

    // Expect 1: URL ต้องเป็น .xml
    expect(
      xmlUrl.includes(".xml"),
      `URL "${xmlUrl}" ไม่ใช่ไฟล์ .xml`,
    ).toBeTruthy();

    // Expect 2: เปิดลิงก์ XML ได้สำเร็จ
    ...
    // Expect 3: Content-Type ต้องเป็น XML
    ...
    // Expect 4: เนื้อหาต้องเป็น XML ที่ถูกต้อง
    ...
    console.log(`📌 XML preview: "${body.slice(0, 150)}"`);
    console.log(`✅ TC-DA-HP-025 ผ่าน — เปิดลิงก์ XML สำเร็จ | URL: ${xmlUrl}`);
  });
  ...
  test("TC-DA-HP-030 - ตรวจสอบปุ่ม Copy ข้อความแจ้งเตือน", async ({
    ...
    const expectedDescription =
      await home.getDescriptionTextFromDetail(detailContainer);
  ...
  test("TC-DA-HP-031 - ตรวจสอบแสดงพื้นที่ polygon", async () => {
    ...
    const polygonLayer = mapContainer
      .locator(HOME_DATA.SELECTORS.mapPolygon)
      .first();
  ...
  test("TC-DA-HP-032 - ตรวจสอบ map แสดงพื้นที่กระทบ", async () => {
    ...
    await expect(
      detailContainer.getByText(HOME_DATA.LABELS.detail.region),
    ).toBeVisible();
    await expect(
      detailContainer.getByText(HOME_DATA.LABELS.detail.province),
    ).toBeVisible();
    await expect(
